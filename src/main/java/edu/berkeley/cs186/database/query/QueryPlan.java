...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        // Create SequentialScanOperator
        QueryOperator minOp = new SequentialScanOperator(this.transaction, table);
        // Set negative value for index
        int i = -1000;
        // Get the eligible index column values (that is a List)
        List<Integer> index_col = getEligibleIndexColumns(table);
        // Iterator through the index column values
        for (int y=0;y<index_col.size();y++){
            // Create an IndexScanOperator using this
            IndexScanOperator indscanop = new IndexScanOperator(this.transaction, table, this.selectPredicates.get(index_col.get(y)).column, this.selectPredicates.get(index_col.get(y)).operator, this.selectPredicates.get(index_col.get(y)).value);
            // Check if the sequential scan operator has a higher IO Cost than the IndexScanOperator
            if (indscanop.estimateIOCost() < minOp.estimateIOCost()){
                // If so, set new index value
                i=index_col.get(y);
        ...
        return addEligibleSelections(minOp, i);
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set table:prevMap.keySet()){
            ...
            for (int i = 0;i<this.joinPredicates.size();++i){
                // Get the particular joinPredicate at index i
                JoinPredicate x = this.joinPredicates.get(i);
                // Set hashset for table
                Set val1 = new HashSet<>(table);
                // Set intermediate hashset
                Set val2 = new HashSet<>();
                // variable vor IO cost
                int ourio;
                // Query operator used
                QueryOperator ourop;
                // The set contains left table but not right
                if (table.contains(x.leftTable) && !table.contains(x.rightTable)){
                    // Add rightTable to table hashset
                    val1.add(x.rightTable);
                    // Add rightTable to intermediate hashset
                    val2.add(x.rightTable);
                    // Find the operator to access the rightTable
                    ourop = minCostJoinType(prevMap.get(table), pass1Map.get(val2), x.leftColumn, x.rightColumn);
                    // Find the IO cost for this call
                    ourio = ourop.estimateIOCost();
                    // The set contains right table but not left
                }else if (table.contains(x.rightTable) && !table.contains(x.leftTable)){
                    // Add leftTable to table hashset
                    val1.add(x.leftTable);
                    // Add leftTable to intermediate hashset
                    val2.add(x.leftTable);
                    // Find the operator to access the leftTable
                    ourop = minCostJoinType(prevMap.get(table), pass1Map.get(val2), x.rightColumn, x.leftColumn);
                    // Find the IO cost for this call
                    ourio = ourop.estimateIOCost();
                // Otherwise, skip this join predicate and continue the loop.
                }else{
                    continue;
                }
                // If either the resulting hashset is not there or its IO cost is lower than the current lowest one
                if (result.get(val1) == null || ourio < result.get(val1).estimateIOCost()){
                    // Place the hashset and its respective operator to the result map
                    result.put(val1,ourop);
                }
            }
        }
        // Return our deduced mapping
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        // Create new pass 1 map
        Map<Set<String>, QueryOperator> p1=new HashMap<>();
        // For each table (from tableNames)
        for (String table:tableNames){
            // Create new hashset for map
            Set<String> ourhs = new HashSet<>();
            // Add table to our HashSet
            ourhs.add(table);
            // Construct a mapping of each table name to its lowest cost operator.
            p1.put(ourhs,minCostSingleAccess(table));
        }
        // Set pass i map to initial value of pass 1
        Map<Set<String>, QueryOperator> pi= p1;
        // Iterate through passes until the pass i size is 1
        while(pi.size() > 1){
            // Find lowest cost joins with each table from pass 1
            pi = minCostJoins(pi, p1);
        }
        // Set final operator to lowest cost operator from the last pass
        finalOperator = minCostOperator(pi);
        // Add groupy operator
        addGroupBy();
