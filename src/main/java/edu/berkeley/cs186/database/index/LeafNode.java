...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException{
        ...
        for (int i = 0; i < keys.size(); ++i){
            if (key.compareTo(keys.get(i)) == 0){throw new BPlusTreeException("Record already exists!");}else if (key.compareTo(keys.get(i)) > 0){x = i+1;}else if(key.compareTo(keys.get(i)) < 0){x = i;break;}
        }
        // keys.update(x,key);
        // keys.update(x,rid);
        keys.add(x,key);
        ...
        if (keys.size() > 2*metadata.getOrder()){
            ...
            List<RecordId> shalfrid = rids.subList(metadata.getOrder(), keys.size());
            List<DataBox> shalfkey = keys.subList(metadata.getOrder(), keys.size());
            ...
            long val = (new LeafNode(metadata,bufferManager,shalfkey,shalfrid,rightSibling,treeContext)).getPage().getPageNum();
            rightSibling = Optional.of(val);

            sync();
            // return Optional.of((new LeafNode(metadata,bufferManager,shalfkey,shalfrid,rightSibling,treeContext)).getPage().getPageNum());
            ret = Optional.of(new Pair<>(shalfkey.get(0), val));}else{sync();}
        // return value.ret();
        return ret;
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) throws BPlusTreeException{
                int count = (int)Math.ceil(2 * metadata.getOrder() * fillFactor);
                Optional<Pair<DataBox,Long>> ret = Optional.empty();
                while (data.hasNext()){
                    Pair<DataBox,RecordId> nextrec = data.next();
                    // keys.update(x,key);
                    // keys.update(x,rid);
                    keys.add(nextrec.getFirst());
                    rids.add(nextrec.getSecond());
                    if (keys.size() <= count){}else{
                        // List<RecordId> shalfrid = rids.subList(0, keys.size());
                        // List<DataBox> shalfkey = keys.subList(0, keys.size());
                        List<RecordId> shalfrid = rids.subList(count, keys.size());
                        List<DataBox> shalfkey = keys.subList(count, keys.size());
                        // keys = keys.subList(metadata.getOrder(),keys.size());
                        // rids = rids.subList(metadata.getOrder(),keys.size());
                        keys = keys.subList(0,count);
                        rids = rids.subList(0,count);
                        long val = (new LeafNode(metadata,bufferManager,shalfkey,shalfrid,rightSibling,treeContext)).getPage().getPageNum();
                        rightSibling = Optional.of(val);
                        sync();
                        // return Optional.of((new LeafNode(metadata,bufferManager,shalfkey,shalfrid,rightSibling,treeContext)).getPage().getPageNum());
                        ret = Optional.of(new Pair<>(shalfkey.get(0), val));
                        break;
                    }
            }
            sync();
