...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        for (int i = 0; i<keys.size();++i){
            // if(numLessThan(key,keys.get(i)<0)){return getChild(i).update(key);}
            if (key.compareTo(keys.get(i)) < 0){return getChild(i).get(key);}
        }
        // return getChild(keys.size()-1).update(key);
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException{
        ...
        for (int i = 0; i < keys.size(); ++i){
            if (key.compareTo(keys.get(i)) >= 0){x = i+1;}else{x = i;break;}
        }
        Optional<Pair<DataBox, Long>> ournode = getChild(x).put(key, rid);
        if (!ournode.equals(Optional.empty())){
            // keys.update(x,key);
            // keys.update(x,rid);
            Long right = ournode.get().getSecond();
            children.add(x+1,right);
            DataBox left = ournode.get().getFirst();
            keys.add(x,left);
        }
        if (keys.size() <= 2*metadata.getOrder()){sync();}else{
            // List<RecordId> shalfrid = rids.subList(0, keys.size());
            // List<DataBox> shalfkey = keys.subList(0, keys.size());
            List<Long> rightH = children.subList(metadata.getOrder() + 1, children.size());
            List<DataBox> leftH = keys.subList(metadata.getOrder() + 1, keys.size());
            InnerNode In_N = new InnerNode(metadata, bufferManager,leftH, rightH,treeContext);
            DataBox newkey = keys.remove(metadata.getOrder());
            // keys = keys.subList(metadata.getOrder(),keys.size());
            // rids = rids.subList(metadata.getOrder(),keys.size());
            children = children.subList(0, metadata.getOrder() + 1);
            keys = keys.subList(0, metadata.getOrder());
            sync();
            Long val = In_N.getPage().getPageNum();
            // return Optional.of((new Pair<>((metadata,bufferManager,shalfkey,shalfrid,rightSibling,treeContext)).getPage().getPageNum(),val);
            ret = Optional.of(new Pair<>(newkey, val));
        }
        return ret;
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        while (data.hasNext()){
            Optional<Pair<DataBox, Long>> ournode = getChild(children.size()-1).bulkLoad(data, fillFactor);
            if (!ournode.equals(Optional.empty())){
                // keys.update(x,key);
                // keys.update(x,rid);
                children.add(ournode.get().getSecond());
                keys.add(ournode.get().getFirst());
                if (keys.size() <= 2*metadata.getOrder()){sync();}else{
                    // List<RecordId> shalfrid = rids.subList(0, keys.size());
                    // List<DataBox> shalfkey = keys.subList(0, keys.size());
                    List<Long> rightH = children.subList(metadata.getOrder() + 1, children.size());
    ...
    public void remove(DataBox key) {
        for (int i = 0; i < keys.size(); ++i){
            // if (key.numLessThan(key,keys.get(i)) < 0){
            //      getChild(i).get(key);
            // }
            if (key.compareTo(keys.get(i)) < 0){getChild(i).remove(key);break;}
        }
        // getChild(i).get(key);
