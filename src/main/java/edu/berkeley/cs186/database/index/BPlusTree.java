...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException{
        typecheck(key);
        LockUtil.ensureSufficientLockHeld(lockContext,LockType.NL);
        Optional<Pair<DataBox,Long>> ourleaf = root.put(key,rid);
        // TODO(proj4_integration): Update the following line
        ...
        if (ourleaf.equals(Optional.empty())){
        }else{
            // InnerNode in_N = ourleaf.get().getRightSibling();
            DataBox left = ourleaf.get().getFirst();
            Long ourpage = root.getPage().getPageNum();
            List<Long> ourchild = new ArrayList<>();
            Long right = ourleaf.get().getSecond();
            // ourchild.update(left);\
            // ourchild.update(right);
            ourchild.add(ourpage);
            ourchild.add(right);
            // ourkey.update(left);
            List<DataBox> ourkey = new ArrayList<>();
            ourkey.add(left);
            updateRoot((new InnerNode(metadata, bufferManager, ourkey, ourchild, lockContext)));
        }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor)  throws BPlusTreeException{
        // TODO(proj4_integration): Update the following line
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.NL);
        if (!data.hasNext() || !this.toSexp().equals("()")) { 
        ...
        // if (data.next() == null){}
        while(data.hasNext()){
        Optional<Pair<DataBox,Long>> ourleaf = root.bulkLoad(data, fillFactor);
        if (ourleaf.equals(Optional.empty())){
        }else{
            // InnerNode in_N = ourleaf.get().getRightSibling();
            DataBox left = ourleaf.get().getFirst();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private Iterator<RecordId> ourid;
        private LeafNode ourleaf;
        public BPlusTreeIterator(){ourleaf = root.getLeftmostLeaf();ourid = ourleaf.scanAll();}
        public BPlusTreeIterator(DataBox ourbox){ourleaf = root.get(ourbox);ourid = ourleaf.scanGreaterEqual(ourbox);}
        @Override
        public boolean hasNext() {
            Optional<LeafNode> right = ourleaf.getRightSibling();
            // if (ourleaf.getRightSibling() == null){return false;};
            if (ourid.hasNext()){return true;}else if (ourid == null){return false;}
            while (!right.equals(Optional.empty())) { 
                // ourid = ourid.next();
                // ourleaf = ourleaf.getRightSibling();
                ourid = right.get().scanAll();
                ourleaf = right.get();
                // if (ourid.next() == null){return false;};
                if (!ourid.hasNext()) {right = right.get().getRightSibling(); }else{return true;}
            }
            return false;
        }
        ...
        public RecordId next() {
            // if (ourid.next() == Optional.empty()){return ret;}
            if (!ourid.hasNext()){throw new NoSuchElementException();}else{return ourid.next();}
        }
    }
}