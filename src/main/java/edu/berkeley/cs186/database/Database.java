...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                // System.out.println(holdLocks);
                while (!lockManager.getLocks(this).isEmpty()){
                    // System.out.println(LockContext.fromResourceName(lockManager,lock.name).getEffectiveLockType(this));
                    LockContext.fromResourceName(lockManager,lockManager.getLocks(this).get(lockManager.getLocks(this).size()-1).name).release(this);
                    // System.out.println(LockContext.fromResourceName(lockManager,lock.name).getEffectiveLockType(this));
                }
                // System.out.println(holdLocks);
                return;
